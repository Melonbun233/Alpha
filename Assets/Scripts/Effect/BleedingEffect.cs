...
public class BleedingEffect : Effect
{
    ...
    public override EffectType type { get; } = EffectType.BleedingEffect;
    ...
    // Period between each bleeding damage dealt the attached unit
    ...
    public BleedingEffect(float damagePeriod, DamageData damageData)
    {
    ...
    public static BleedingEffect deepCopy(BleedingEffect effect)
    {
        return new BleedingEffect(effect.damagePeriod, DamageData.deepCopy(effect.damageData));
    }

    public override void applyEffect(Unit unit)
    {
        // Bleeding effect is always stackable, we dont check it anyway
        unit.effectData.effects.Add(this);
        unit.effectData.bleedingStack++;
    ...
    private void OnUpdate(Unit unit, float deltaTime)
    {
        ...
        // check if we can deal another bleeding damage
        damagePeriodCD -= deltaTime;
        if (damagePeriodCD <= 0)
        {
            unit.receiveDamage(damageData, null);
            unit.healthData.healingPercentMultiplier /= 2;
